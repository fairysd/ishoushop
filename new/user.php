...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer', 'oath' , 'oath_login', 'other_login','is_registered_byphone');

/* 显示页面的action列表 */
$ui_arr = array('estimate_update_expressno', 'estimate_cancel', 'mortgageintention_cancel', 'mortgageintention_list', 'mortgageintention_list_detail', 'mortgage_list', 'mortgage_list_detail', 'estimate_list', 'estimate_list_detail', 'register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $agreement = isset($_POST['agreement']) ? trim($_POST['agreement']) : trim($_GET['agreement']);

        //donny
        $username = $other['mobile_phone'];
    
        $ajax = isset($_POST['ajax']) ? trim($_POST['ajax']) : $_GET['ajax'];
        if ($ajax == "1") {
        ...
        {
            if ($ajax) {
                $res = array("success"=>false, "msg"=>$_LANG['passport_js']['agreement']);
        ...
        {
            if ($ajax) {
                $res = array("success"=>false, "msg"=>$_LANG['passport_js']['username_shorter']);
                die($callback . '(' . $json->encode($res) .')');
            } else {
        ...
        {
            if ($ajax) {
                $res = array("success"=>false, "msg"=>$_LANG['passport_js']['password_shorter']);
                die($callback . '(' . $json->encode($res) .')');
            } else {
        ...
        {
            if ($ajax) {
                $res = array("success"=>false, "msg"=>$_LANG['passwd_balnk']);
                die($callback . '(' . $json->encode($res) .')');
            } else {
        ...
        if ((intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0)
        {
            ...
            {
                if ($ajax) {
                    $res = array("success"=>false, "msg"=>$_LANG['invalid_captcha']);
                    die($callback . '(' . $json->encode($res) .')');
                } else {
        ...
        // donny for test
         // show_message_register($other['mobile_phone']);
         // exit;
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            if ($ajax) {
                ...
                show_message_register($other['mobile_phone'], $back_act); 
            }
        }
        else
        {
            if ($ajax) {
                $res = array("success"=>false, "msg"=>"注册失败!");
                die($callback . '(' . $json->encode($res) .')');
            } else {
...
/* 通过手机号验证用户是否可以注册 --donny */
elseif ($action == 'is_registered_byphone')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    include_once(ROOT_PATH . 'includes/lib_passport.php');
    ...
    $ajax = isset($_POST['ajax']) ? trim($_POST['ajax']) : $_GET['ajax'];
    $callback = isset($_GET['callback']) ? trim($_GET['callback']) : '';
    if ($user->check_moblephone($phone))
    {
        if ($ajax) {
            include_once(ROOT_PATH . 'includes/cls_json.php');
            $json = new JSON;
...
elseif ($action == 'act_login')
{
    ...
    else
    {
        if ($ajax) {
            ...
            $json = new JSON;
            header('Content-type:text/json'); 
            $res = array("success"=>false, "msg"=>"用户名/密码错误!");
            die($callback . '(' . $json->encode($res) .')');
        }
        $_SESSION['login_fail'] ++ ;
...
elseif ($action == 'logout')
{
    ...
    $user->logout();
    $ucdata = empty($user->ucdata)? "" : $user->ucdata;
    show_message($_LANG['logout'] . $ucdata, array($_LANG['back_up_page'], $_LANG['back_home_lnk']), array($back_act, 'http://www.aishoubao.com'), 'info');
}
...
elseif ($action == 'act_edit_profile')
{
    ...
    $sel_question = empty($_POST['sel_question']) ? '' : compile_str($_POST['sel_question']);
    $passwd_answer = isset($_POST['passwd_answer']) ? compile_str(trim($_POST['passwd_answer'])) : '';
	$people_img   = (isset($_FILES['people_img']['error']) && $_FILES['people_img']['error'] == 0) || (!isset($_FILES['people_img']['error']) && isset($_FILES['people_img']['tmp_name']) && $_FILES['people_img']['tmp_name'] != 'none')
         ? $_FILES['people_img'] : array() ;
	if(!empty($people_img)){
		$other['people_img'] = $people_img;
	}
    ...
    if (!empty($cardno))
    {
        $sql = 'UPDATE ' . $ecs->table('users') . " SET `cardno`='$cardno' WHERE `user_id`='" . $_SESSION['user_id'] . "'";
        $db->query($sql);
    }
...
/* 回收订单管理 -donny */
elseif ($action == 'estimate_list')
{
    include_once(ROOT_PATH . 'includes/requests/Requests.php');
    Requests::register_autoloader();
    $apiAddr = 'http://114.215.207.99:8888/recycle-api/api/myintention/'.$_SESSION['mobile_phone'];
    $response = Requests::get($apiAddr);
    $res = json_decode($response->body, true);
    $estimateOrds = $res["body"];
    foreach ($estimateOrds as $key => $ord) {
        $estimateOrds[$key]["createTime"] = date('Y-m-d H:i:s', $ord["createTime"]/1000);
        if ($ord["status"]=="1") {
            $estimateOrds[$key]["status"] = "等待验货";
        }
        if ($ord["status"]=="2") {
            $estimateOrds[$key]["status"] = "交易成功";
        }
        if ($ord["status"]=="3") {
            $estimateOrds[$key]["status"] = "平台取消交易";
        }
        if ($ord["status"]=="4") {
            $estimateOrds[$key]["status"] = "用户取消交易";
        }
    }
    
    $smarty->assign('estimateOrds', $estimateOrds);
    $smarty->display('user_estimate.dwt');
}
/* 回收订单取消 -donny */
elseif ($action == 'estimate_cancel')
{
    include_once(ROOT_PATH . 'includes/requests/Requests.php');
    Requests::register_autoloader();
    $apiAddr = 'http://114.215.207.99:8888/recycle-api/api/intentionreject/'.$_GET["id"];
    $response = Requests::get($apiAddr);
    $res = json_decode($response->body, true);

    header("Location:user.php?act=estimate_list");
    exit;
}
/* 回收订单设置快递单号 */
elseif ($action == 'estimate_update_expressno')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    include_once(ROOT_PATH . 'includes/requests/Requests.php');
    Requests::register_autoloader();
    $apiAddr = 'http://114.215.207.99:8888/recycle-api/api/intentionSubmitExpressNo/'.$_GET["id"].'/'.$_GET['expressno'];
    $response = requests::get($apiAddr);
    header('Content-type:text/json'); 
    die($json->encode($response->body));
}
/* 抵押意向订单取消 -donny */
elseif ($action == 'mortgageintention_cancel')
{
    include_once(ROOT_PATH . 'includes/requests/Requests.php');
    Requests::register_autoloader();
    $apiAddr = 'http://114.215.207.99:8888/recycle-api/api/mortgageintentionreject/'.$_GET["id"];
    $response = Requests::get($apiAddr);
    $res = json_decode($response->body, true);

    header("Location:user.php?act=mortgageintention_list");
    exit;
}
/* 回收订单详情 -donny */
elseif ($action == 'estimate_list_detail')
{
    include_once(ROOT_PATH . 'includes/requests/Requests.php');
    Requests::register_autoloader();
    $apiAddr = 'http://114.215.207.99:8888/recycle-api/api/intentiondetail/'.$_GET["id"];
    $response = Requests::get($apiAddr);
    $res = json_decode($response->body, true);
    $estimateDetail = $res["body"];

    if ($estimateDetail["status"]=="1") {
            $estimateDetail["status"] = "等待验货";
    }
    if ($estimateDetail["status"]=="2") {
        $estimateDetail["status"] = "交易成功";
    }
    if ($estimateDetail["status"]=="3") {
        $estimateDetail["status"] = "平台取消交易";
    }
    if ($estimateDetail["status"]=="4") {
        $estimateDetail["status"] = "用户取消交易";
    }

    $smarty->assign('estimateDetail', $estimateDetail);
    $smarty->display('user_estimate.dwt');
}
/* 抵押意向订单 -donny */
elseif ($action == 'mortgageintention_list')
{
    include_once(ROOT_PATH . 'includes/requests/Requests.php');
    Requests::register_autoloader();
    $apiAddr = 'http://114.215.207.99:8888/recycle-api/api/mymortgageintention/'.$_SESSION['mobile_phone'];
    $response = Requests::get($apiAddr);
    $res = json_decode($response->body, true);
    // $res = json_decode('{"body":[{"accountName":"","bankCardno":"","bankName":"","channelId":"","createTime":1460006120000,"defaultPicUrl":"http://7xonzf.com1.z0.glb.clouddn.com/product/20160119140653_0553.jpg","diamondRemark":"","estimatePrice":735.00,"id":"63f7665169ad44f686b5a50070619cf8","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageInterestDay":5.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","status":"1","tradeNo":"2016040713151988648","tradeType":"1","unitName":"克"},{"accountName":"","bankCardno":"","bankName":"","channelId":"","createTime":1460006063000,"defaultPicUrl":"http://7xonzf.com1.z0.glb.clouddn.com/product/20160119140653_0553.jpg","diamondRemark":"","estimatePrice":735.00,"id":"c1510040da114845a91699401af62c9c","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageInterestDay":5.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","status":"1","tradeNo":"2016040713142290330","tradeType":"1","unitName":"克"},{"accountName":"13390886239","bankCardno":"","bankName":"","channelId":"","createTime":1460006031000,"defaultPicUrl":"http://7xonzf.com1.z0.glb.clouddn.com/product/20160119140653_0553.jpg","diamondRemark":"","estimatePrice":735.00,"id":"311d3df4f2804207be6ae1b4fe69b8be","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageInterestDay":5.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","status":"1","tradeNo":"2016040713135155290","tradeType":"1","unitName":"克"}],"success":true}', true);
    $mortgages = $res["body"];

    foreach ($mortgages as $key => $ord) {
        $mortgages[$key]["createTime"] = date('Y-m-d H:i:s', $ord["createTime"]/1000);
        $mortgages[$key]["mortgageStarttime"] = date('Y-m-d', $ord["mortgageStarttime"]/1000);
        $mortgages[$key]["mortgageEndtime"] = date('Y-m-d', $ord["mortgageEndtime"]/1000);
        if ($ord["status"]=="1") {
            $mortgages[$key]["status"] = "等待处理";
        }
        if ($ord["status"]=="2") {
            $mortgages[$key]["status"] = "已处理";
        }
        if ($ord["status"]=="4") {
            $mortgages[$key]["status"] = "已取消";
        }
    }

    $smarty->assign('mortgages', $mortgages);
    $smarty->display('user_estimate.dwt');
}
/* 抵押意向订单详情 -donny */
elseif ($action == 'mortgageintention_list_detail')
{
    include_once(ROOT_PATH . 'includes/requests/Requests.php');
    Requests::register_autoloader();
    $apiAddr = 'http://114.215.207.99:8888/recycle-api/api/mortgageintentiondetail/'.$_GET["id"];
    $response = Requests::get($apiAddr);
    $res = json_decode($response->body, true);
    // $res = json_decode('{"body":{"accountName":"","bankCardno":"","bankName":"","channelId":"","channelName":"","createTime":1460006120000,"diamondRemark":"","estimatePrice":735.00,"fullPropertyOptions":"品质:金摆件","id":"63f7665169ad44f686b5a50070619cf8","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageInterestDay":5.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"orgAddress":"苏州市新区泉屋百货","orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","orgTel":"12345678","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"parentCategoryId":"47f288756f1a4c5ebac0a2baf1f5ee80","payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","propertyOptions":"金摆件","status":"1","tradeNo":"2016040713151988648","tradeType":"1","unitName":"克"},"success":true}',true);
    $mortgageDetail = $res["body"];

    $mortgageDetail["createTime"] = date('Y-m-d H:i:s', $mortgageDetail["createTime"]/1000);
    $mortgageDetail["mortgageStarttime"] = date('Y-m-d', $mortgageDetail["mortgageStarttime"]/1000);
    $mortgageDetail["mortgageEndtime"] = date('Y-m-d', $mortgageDetail["mortgageEndtime"]/1000);
    if ($mortgageDetail["status"]=="1") {
        $mortgageDetail["status"] = "等待处理";
    }
    if ($mortgageDetail["status"]=="2") {
        $mortgageDetail["status"] = "已处理";
    }
    if ($mortgageDetail["status"]=="4") {
        $mortgageDetail["status"] = "已取消";
    }

    $smarty->assign('mortgageDetail', $mortgageDetail);
    $smarty->display('user_estimate.dwt');
}
/* 抵押订单 -donny */
elseif ($action == 'mortgage_list')
{
    include_once(ROOT_PATH . 'includes/requests/Requests.php');
    Requests::register_autoloader();
    $apiAddr = 'http://114.215.207.99:8888/recycle-api/api/mymortgage/'.$_SESSION['mobile_phone'];
    $response = Requests::get($apiAddr);
    $res = json_decode($response->body, true);
    // $res = json_decode('{"body":[{"accountName":"","bankCardno":"","bankName":"","channelId":"","createTime":1460006158000,"diamondRemark":"","estimatePrice":735.00,"id":"820e8f393b5d49d18c7c72e3dd0de197","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageExpectEarnings":294.00,"mortgageRealInterestDay":6.00,"mortgageRealPrice":700.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"operateUserId":"0","operateUserName":"管理员","orgAddress":"苏州市新区泉屋百货","orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","orgTel":"12345678","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","remark":"","status":"1","tradeNo":"2016040713155825518","tradeType":"1"},{"accountName":"13390886239","bankCardno":"","bankName":"","channelId":"","createTime":1460000270000,"diamondRemark":"","estimatePrice":735.00,"id":"c4af678219a44163850287d89561ca6f","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageExpectEarnings":257.25,"mortgageRealInterestDay":5.00,"mortgageRealPrice":735.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"operateUserId":"0","operateUserName":"管理员","orgAddress":"苏州市新区泉屋百货","orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","orgTel":"12345678","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","remark":"","status":"1","tradeNo":"2016040711375071256","tradeType":"1"},{"accountName":"13390886239","bankCardno":"","bankName":"","channelId":"","createTime":1460000140000,"diamondRemark":"","estimatePrice":735.00,"id":"d0189c7bccd241fcb788a83e91bf5079","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageExpectEarnings":257.25,"mortgageRealInterestDay":5.00,"mortgageRealPrice":735.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"operateUserId":"0","operateUserName":"管理员","orgAddress":"苏州市新区泉屋百货","orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","orgTel":"12345678","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","remark":"","status":"1","tradeNo":"2016040711354088421","tradeType":"1"},{"accountName":"13390886239","bankCardno":"","bankName":"","channelId":"","createTime":1459999997000,"diamondRemark":"","estimatePrice":735.00,"id":"6690b39c5621400089c6a61bb7e383f0","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageExpectEarnings":257.25,"mortgageRealInterestDay":5.00,"mortgageRealPrice":735.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"operateUserId":"0","operateUserName":"管理员","orgAddress":"苏州市新区泉屋百货","orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","orgTel":"12345678","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","remark":"","status":"1","tradeNo":"2016040711331700526","tradeType":"1"},{"accountName":"13390886239","bankCardno":"","bankName":"","channelId":"","createTime":1459999677000,"diamondRemark":"","estimatePrice":735.00,"id":"12ab893f8abf44189dc66f9af75ef197","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageExpectEarnings":257.25,"mortgageRealInterestDay":5.00,"mortgageRealPrice":735.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"operateUserId":"0","operateUserName":"管理员","orgAddress":"苏州市新区泉屋百货","orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","orgTel":"12345678","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","remark":"","status":"1","tradeNo":"2016040711275786325","tradeType":"1"},{"accountName":"13390886239","bankCardno":"","bankName":"","channelId":"","createTime":1459999402000,"diamondRemark":"","estimatePrice":735.00,"id":"5f2a41872f2949808889397b5e52c3ab","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1461254400000,"mortgageExpectEarnings":551.25,"mortgageRealInterestDay":5.00,"mortgageRealPrice":735.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"operateUserId":"0","operateUserName":"管理员","orgAddress":"苏州市新区泉屋百货","orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","orgTel":"12345678","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"4","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","remark":"","status":"1","tradeNo":"2016040711232108026","tradeType":"1"},{"accountName":"","bankCardno":"","bankName":"","channelId":"","createTime":1459999210000,"diamondRemark":"","estimatePrice":735.00,"id":"b481d0872a904ecc9329516edab5b833","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1461772800000,"mortgageExpectEarnings":735.00,"mortgageRealEarnings":735.00,"mortgageRealInterestDay":5.00,"mortgageRealPrice":735.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"operateUserId":"0","operateUserName":"管理员","orgAddress":"苏州市新区泉屋百货","orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","orgTel":"12345678","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","remark":"","status":"3","tradeNo":"2016040711201028910","tradeType":"1"}],"success":true}', true);
    $mortgages = $res["body"];
    ...
    foreach ($mortgages as $key => $ord) {
        ...
        $mortgages[$key]["mortgageEndtime"] = date('Y-m-d', $ord["mortgageEndtime"]/1000);
        if ($ord["status"]=="1") {
            $mortgages[$key]["status"] = "抵押中";
        }
        if ($ord["status"]=="2") {
            $mortgages[$key]["status"] = "抵押完成";
        }
        if ($ord["status"]=="3") {
            $mortgages[$key]["status"] = "抵押关闭";
        }
        if ($ord["status"]=="4") {
            $mortgages[$key]["status"] = "死当";
        }
        if ($ord["status"]=="5") {
            $mortgages[$key]["status"] = "延期";
        }
        if ($ord["status"]=="6") {
            $mortgages[$key]["status"] = "系统延期";
        }
    }
...
/* 抵押订单详情 -donny */
elseif ($action == 'mortgage_list_detail')
{
    include_once(ROOT_PATH . 'includes/requests/Requests.php');
    Requests::register_autoloader();
    $apiAddr = 'http://114.215.207.99:8888/recycle-api/api/mortgagedetail/'.$_GET["id"];
    $response = Requests::get($apiAddr);
    $res = json_decode($response->body, true);
    // $res = json_decode('{"body":{"mortgageLogs":[{"id":63,"logMortgageEndtime":1460563200000,"logMortgageExpectEarnings":294.00,"logMortgageRealInterestDay":6.00,"logMortgageStarttime":1459958400000,"logMortgageStatus":"1","operateCmd":4,"operateComment":"","operateTime":1460010916000,"operateUserId":"0","operateUserName":"管理员","tradeNo":"2016040713155825518"}],"mortgagePage":{"accountName":"","bankCardno":"","bankName":"","channelId":"","channelName":"","createTime":1460006158000,"diamondRemark":"","estimatePrice":735.00,"fullPropertyOptions":"品质:金摆件","id":"820e8f393b5d49d18c7c72e3dd0de197","isDiamond":0,"memberId":"815d849a8bc24309bc5ea63dca1b4ead","mobile":"13390886239","mortgageEndtime":1460563200000,"mortgageExpectEarnings":294.00,"mortgageRealInterestDay":6.00,"mortgageRealPrice":700.00,"mortgageStarttime":1459958400000,"name":"donny","noblemetal":false,"num":3.00,"operateUserId":"0","operateUserName":"管理员","orgAddress":"苏州市新区泉屋百货","orgId":"f0b531becda44fb68472500171b488bf","orgName":"新区泉屋","orgTel":"12345678","pageInfo":{"checkFirstFrom":true,"page":1,"rows":10},"parentCategoryId":"47f288756f1a4c5ebac0a2baf1f5ee80","payType":"1","productId":"8fa078e445184f1f9041ea1be763a5e8","productName":"黄金","propertyOptions":"金摆件","remark":"","status":"1","tradeNo":"2016040713155825518","tradeType":"1"}},"success":true}',true);
    $mortgageDetail = $res["body"]["mortgagePage"];
    $mortgageLogs = $res["body"]["mortgageLogs"];

    $mortgageDetail["createTime"] = date('Y-m-d H:i:s', $mortgageDetail["createTime"]/1000);
    $mortgageDetail["mortgageStarttime"] = date('Y-m-d', $mortgageDetail["mortgageStarttime"]/1000);
    $mortgageDetail["mortgageEndtime"] = date('Y-m-d', $mortgageDetail["mortgageEndtime"]/1000);
    if ($mortgageDetail["status"]=="1") {
        $mortgageDetail["status"] = "抵押中";
    }
    if ($mortgageDetail["status"]=="2") {
        $mortgageDetail["status"] = "抵押完成";
    }
    if ($mortgageDetail["status"]=="3") {
        $mortgageDetail["status"] = "抵押关闭";
    }
    if ($mortgageDetail["status"]=="4") {
        $mortgageDetail["status"] = "死当";
    }
    if ($mortgageDetail["status"]=="5") {
        $mortgageDetail["status"] = "延期";
    }
    if ($mortgageDetail["status"]=="6") {
        $mortgageDetail["status"] = "系统延期";
    }

    foreach ($mortgageLogs as $key => $log) {
        $mortgageLogs[$key]["logMortgageStarttime"] = date('Y-m-d H:i:s', $log["logMortgageStarttime"]/1000);
        $mortgageLogs[$key]["logMortgageEndtime"] = date('Y-m-d', $log["logMortgageEndtime"]/1000);
        $mortgageLogs[$key]["operateTime"] = date('Y-m-d', $log["operateTime"]/1000);
        if ($log["operateCmd"]=="1") {
            $mortgageLogs[$key]["operateCmd"] = "延期";
        }
        if ($log["operateCmd"]=="2") {
            $mortgageLogs[$key]["operateCmd"] = "结算";
        }
        if ($log["operateCmd"]=="3") {
            $mortgageLogs[$key]["operateCmd"] = "死当";
        }
        if ($log["operateCmd"]=="4") {
            $mortgageLogs[$key]["operateCmd"] = "确认抵押";
        }
        if ($log["operateCmd"]=="6") {
            $mortgageLogs[$key]["operateCmd"] = "系统延期";
        }
    }

    $smarty->assign('mortgageDetail', $mortgageDetail);
    $smarty->assign('mortgageLogs', $mortgageLogs);
    $smarty->display('user_estimate.dwt');
}
?>