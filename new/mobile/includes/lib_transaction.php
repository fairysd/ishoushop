...
function get_url_contents($url){
	if (function_exists('curl_init') == 1){
	  ...
	  curl_close ($curl);
		...
		include_once(ROOT_PATH . 'plugins/kuaidi100/snoopy.php');
	  $snoopy = new snoopy();
	  $snoopy->fetch($url);
...
{
	$sql = "SELECT o.goods_id, goods_thumb FROM " .$GLOBALS['ecs']->table('order_goods') ." AS o LEFT JOIN " .$GLOBALS['ecs']->table('goods') ." AS g ".
				 " ON o.goods_id=g.goods_id Where o.order_id='$order_id' Limit 1";
	$row = $GLOBALS['db']->GetRow($sql);
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['handler'] = "<a class=\"c-btn4\" href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                @$row['handler'] = "<a class=\"c-btn4\" href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$GLOBALS['_LANG']['received']."</a>";
            }
            elseif ($row['shipping_status'] == SS_RECEIVED)
            {
                @$row['handler'] = '<span class="c-btn6" style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
            }
            else
            {
                if ($row['pay_status'] == PS_UNPAYED)
                {
                    @$row['handler'] = "<a class=\"c-btn4\" href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>';
                }
                else
                {
                    @$row['handler'] = "<a class=\"c-btn4\" href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a>';
                }
        ...
        {
            $row['handler'] = '<span class="c-btn6" style="color:red">'.$GLOBALS['_LANG']['os'][$row['order_status']] .'</span>';
        }
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'order_thumb'    => get_order_thumb($row['order_id']),
