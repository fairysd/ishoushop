...
if ($_REQUEST['act'] == 'list')
{
    ...
    if (!$smarty->is_cached('exchange.dwt', $cache_id))
    {
        ...
        $smarty->assign('integral_max',     $integral_max);
        $smarty->assign('integral_min',     $integral_min);
				$smarty->assign('nextdisplay',      $nextdisplay);
				$smarty->assign('nextcss',          $nextcss);
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id))
    {
        ...
        else
        {
            ...
            $smarty->assign('pictures',            get_goods_gallery($goods_id));                    // 商品相册
						$smarty->assign('comments_count',          get_comment_num($goods_id)); // by  YMB $comments_count 评论数
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    /* 更新：清空购物车中所有团购商品 */
    include_once(ROOT_PATH_MOBILE . 'includes/lib_order.php');
...
function get_exchange_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        $row['goods_thumb'] = get_image_path($goods_id, $row['goods_thumb'], true);
				$row['commnet_count'] = get_comment_num($goods_id);
