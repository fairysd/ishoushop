...
if ($_REQUEST['act'] == 'install')
{
    ...
    if ($_GET['code'] == 'ecshop')
    {
        $sql = "UPDATE " .$ecs->table('shop_config_wap'). " SET value = 'ecshop' WHERE code = 'integrate_code'";
        $db->query($sql);
        $sql = "UPDATE " . $GLOBALS['ecs']->table('shop_config_wap') . " SET value = '' WHERE code = 'points_rule'";
...
if ($_REQUEST['act'] == 'task')
{
    ...
    else
    {
        ...
        /* 插入code到shop_config表 */
        $sql = "SELECT COUNT(*) FROM " .$ecs->table('shop_config_wap'). " WHERE code = 'integrate_code'";

        if ($db->GetOne($sql) == 0)
        {
            $sql = "INSERT INTO " .$ecs->table('shop_config_wap'). " (code, value) ".
                    "VALUES ('integrate_code', '$_SESSION[code]')";
        }
        else
        {
            $sql = "UPDATE " .$ecs->table('shop_config_wap'). " SET value = '$_SESSION[code]' WHERE code = 'integrate_code'";
        }
        $db->query($sql);
...
if ($_REQUEST['act'] == 'setup_ucenter')
{
    ...
    $app_type   = 'ECSHOP';
    $app_name   = $db->getOne('SELECT value FROM ' . $ecs->table('shop_config_wap') . " WHERE code = 'shop_name'");
...
if ($_REQUEST['act'] == 'edit_points')
{
    ...
    $sql = "UPDATE " . $ecs->table('shop_config_wap') . " SET value ='" . serialize($rule) . "' WHERE code='points_rule'";
...
if ($_REQUEST['act'] == 'save_points')
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('shop_config_wap') . " WHERE code='points_set'";
    if ($db->getOne($sql) == 0)
    {
        $sql = "INSERT INTO " . $ecs->table('shop_config_wap') . " (parent_id, type, code, value) VALUES (6, 'hidden', 'points_set', '" . serialize($cfg) . "')";
    }
    else
    {
        $sql = "UPDATE " . $ecs->table('shop_config_wap') . " SET value ='" . serialize($cfg) . "' WHERE code='points_set'";
    }
    $db->query($sql);
    clear_cache_files();
    sys_msg($_LANG['save_ok'],0, array(array('text'=>$_LANG['06_list_integrate'],'href'=>'integrate.php?act=list')));
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('shop_config_wap'). " WHERE code = 'integrate_code'";
    ...
    {
        $sql = "SELECT value FROM " . $GLOBALS['ecs']->table('shop_config_wap') . " WHERE code = 'integrate_code'";
        ...
        $sql = "UPDATE " .$GLOBALS['ecs']->table('shop_config_wap'). " SET value = '$code' WHERE code = 'integrate_code'";
    }
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('shop_config_wap') . " WHERE code = 'integrate_config'";
    if ($GLOBALS['db']->GetOne($sql) == 0)
    {
        $sql =  "INSERT INTO " . $GLOBALS['ecs']->table('shop_config_wap') . " (code, value) ".
                "VALUES ('integrate_config', '" . serialize($cfg) . "')";
    }
    else
    {
        $sql = "UPDATE " . $GLOBALS['ecs']->table('shop_config_wap') . " SET value='". serialize($cfg) ."' ".
