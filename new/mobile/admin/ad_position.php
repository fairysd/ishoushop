...
$exc = new exchange($ecs->table("ad_position_wap"), $db, 'position_id', 'position_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        /* 将广告位置的信息插入数据表 */
        $sql = 'INSERT INTO '.$ecs->table('ad_position_wap').' (position_name, ad_width, ad_height, position_desc, position_style) '.
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告位数据 */
    $sql = 'SELECT * FROM ' .$ecs->table('ad_position_wap'). " WHERE position_id='$id'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    /* 查看广告位是否与其它有重复 */
    $sql = 'SELECT COUNT(*) FROM ' .$ecs->table('ad_position_wap').
           " WHERE position_name = '$position_name' AND position_id <> '$position_id'";
    if ($db->getOne($sql) == 0)
    {
        $sql = "UPDATE " .$ecs->table('ad_position_wap'). " SET ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 查询广告位下是否有广告存在 */
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('ad_wap'). " WHERE position_id = '$id'";
...
function ad_position_list()
{
    ...
    /* 记录总数以及页数 */
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('ad_position_wap');
    ...
    /* 查询数据 */
    $arr = array();
    $sql = 'SELECT * FROM ' .$GLOBALS['ecs']->table('ad_position_wap'). ' ORDER BY position_id DESC';
