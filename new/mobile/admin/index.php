...
elseif ($_REQUEST['act'] == 'main_api')
{
    ...
    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
        ...
        $ecs_template = $db->getOne('SELECT value FROM ' . $ecs->table('shop_config_wap') . ' WHERE code = \'template\'');
        $style = $db->getOne('SELECT value FROM ' . $ecs->table('shop_config_wap') . ' WHERE code = \'stylename\'');
        if($style == '')
        {
            $style = '0';
        }
...
elseif ($_REQUEST['act'] == 'first')
{
    ...
    $sql = 'SELECT value from ' . $ecs->table('shop_config_wap') . " WHERE code='shop_name'";
    $shop_name = $db->getOne($sql);

    $smarty->assign('shop_name', $shop_name);

    $sql = 'SELECT value from ' . $ecs->table('shop_config_wap') . " WHERE code='shop_title'";
...
elseif ($_REQUEST['act'] == 'second')
{
    ...
    if(!empty($shop_name))
    {
        $sql = 'UPDATE ' . $ecs->table('shop_config_wap') . " SET value = '$shop_name' WHERE code = 'shop_name'";
        $db->query($sql);
    }

    if(!empty($shop_title))
    {
        $sql = 'UPDATE ' . $ecs->table('shop_config_wap') . " SET value = '$shop_title' WHERE code = 'shop_title'";
        $db->query($sql);
    }

    if(!empty($shop_address))
    {
        $sql = 'UPDATE ' . $ecs->table('shop_config_wap') . " SET value = '$shop_address' WHERE code = 'shop_address'";
        $db->query($sql);
    }

    if(!empty($shop_country))
    {
        $sql = 'UPDATE ' . $ecs->table('shop_config_wap') . "SET value = '$shop_country' WHERE code='shop_country'";
        $db->query($sql);
    }

    if(!empty($shop_province))
    {
        $sql = 'UPDATE ' . $ecs->table('shop_config_wap') . "SET value = '$shop_province' WHERE code='shop_province'";
        $db->query($sql);
    }

    if(!empty($shop_city))
    {
        $sql = 'UPDATE ' . $ecs->table('shop_config_wap') . "SET value = '$shop_city' WHERE code='shop_city'";
        $db->query($sql);
    }
