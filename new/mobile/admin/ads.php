...
$exc   = new exchange($ecs->table("ad_wap"), $db, 'ad_id', 'ad_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 查看广告名称是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('ad_wap'). " WHERE ad_name = '$ad_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$ecs->table('ad_wap'). " (position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled)
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告数据 */
    $sql = "SELECT * FROM " .$ecs->table('ad_wap'). " WHERE ad_id='".intval($_REQUEST['id'])."'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    /* 更新信息 */
    $sql = "UPDATE " .$ecs->table('ad_wap'). " SET ".
...
function get_adslist()
{
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('ad_wap'). ' AS ad ' . $where;
    ...
    $sql = 'SELECT ad.*, COUNT(o.order_id) AS ad_stats, p.position_name '.
            'FROM ' .$GLOBALS['ecs']->table('ad_wap'). 'AS ad ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('ad_position_wap'). ' AS p ON p.position_id = ad.position_id '.
