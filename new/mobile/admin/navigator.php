...
$exc = new exchange($ecs->table("nav_wap"), $db, 'id', 'name');
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $item_type = $_REQUEST['item_type'];
				$img_name = basename($image->upload_image($_FILES['icon'],'icon'));
				

        $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $ecs->table('nav_wap') . " WHERE type = '". $item_type ."'");
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav_wap') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type,icon) VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$img_name')";
            }
        }

        if(empty($sql))
        {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav_wap') . " (name,ifshow,vieworder,opennew,url,type,icon) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$img_name')";
        }
        $db->query($sql);
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_REQUEST['step']))
    {
        ...
        $row = $db->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('nav_wap') . " WHERE id='$id'");
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $row = $db->getRow("SELECT ctype,cid,ifshow,type FROM " . $GLOBALS['ecs']->table('nav_wap') . " WHERE id = '$id'");
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('nav_wap') .
                " SET name='$item_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type' ";
        }
        else
        {
            //目标不是分类
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('nav_wap') .
                " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type' ";
        }
				
				if (!empty($img_name))
				{
					$sql .= " ,icon = '$img_name' WHERE id='$id' ";
				} else {
					$sql .= " WHERE id='$id' ";
				}
...
{
    $id = (int)$_GET['id'];
    $row = $db->getRow("SELECT ctype,cid,type FROM " . $GLOBALS['ecs']->table('nav_wap') . " WHERE id = '$id' LIMIT 1");
    ...
    $sql = " DELETE FROM " . $GLOBALS['ecs']->table('nav_wap') . " WHERE id='$id' LIMIT 1";
    $db->query($sql);
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'drop_logo')
{
    ...
    /* 取得logo名称 */
    $sql = "SELECT icon FROM " .$ecs->table('nav_wap'). " WHERE id = '$nav_id'";
    $logo_name = $db->getOne($sql);

    if (!empty($logo_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/icon/' .$logo_name);
        $sql = "UPDATE " .$ecs->table('nav_wap'). " SET icon = '' WHERE id = '$nav_id'";
        $db->query($sql);
    }
		clear_cache_files();
		$links[] = array('text' => $_LANG['navigator'], 'href' => 'navigator.php?act=list');
...
if ($_REQUEST['act'] == 'toggle_ifshow')
{
    ...
    $row = $db->getRow("SELECT type,ctype,cid FROM " . $GLOBALS['ecs']->table('nav_wap') . " WHERE id = '$id' LIMIT 1");
...
function get_nav()
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('nav_wap');
        ...
        /* 查询 */
        $sql = "SELECT id, name, ifshow, vieworder, opennew, url, type, icon".
               " FROM ".$GLOBALS['ecs']->table('nav_wap').
...
function nav_update($id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('nav_wap'), $args, 'update', "id='$id'");
}
