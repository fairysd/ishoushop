...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'asynclist')
{
    ...
    $goods_list = brand_get_goods2($brand_id, $cate, $amount, $last, $sort, $order); 
		foreach($goods_list as $key=>$var) 
		{
			 $sayList[] = array(				
				'pro-inner'=>'<div class="proImg-wrap"><a href="'.$var['url'].'"><img alt="'.$var['goods_name'].'" src="'.$var['goods_img'].'"></a></div><div class="proInfo-wrap"><div class="proTitle"><a href="'.$var['url'].'">'.$var['goods_name'].'</a></div><div class="proSKU"></div><div class="proPrice"><em>'.$var['price_str'].'</em></div><div class="proService"><del>'.$var['market_price'].'</del></div><div class="proSales">月销:<em>'.$var['sales_count'].'</em></div><div class="proIcons">'.$var['watermark_str'].'</div></div>'
				);
		}
