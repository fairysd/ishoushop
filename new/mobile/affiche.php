...
if ($_GET['act'] == 'js')
{
    ...
    $sql = 'SELECT ad.ad_id, ad.ad_name, ad.ad_link, ad.ad_code '.
           'FROM ' . $ecs->table('ad_wap') . ' AS ad ' .
           'LEFT JOIN ' . $ecs->table('ad_position_wap') . ' AS p ON ad.position_id = p.position_id '.
...
else
{
    ...
    {
        /* 更新站内广告的点击次数 */
        $db->query('UPDATE ' . $ecs->table('ad_wap') . " SET click_count = click_count + 1 WHERE ad_id = '$ad_id'");
