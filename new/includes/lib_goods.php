...
function get_hot_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.comments_number, g.market_price, g.shop_price , g.promote_price ' .
           'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
           "WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.is_hot=1 $where  " ;
    //判断是否启用库存，库存数量是否大于0
    if ($GLOBALS['_CFG']['use_storage'] == 1)
    {
        $sql .= " AND g.goods_number > 0 ";
    }
    $sql .= ' ORDER BY goods_number DESC, g.goods_id DESC LIMIT 10' ;
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
	...
	$cat['cat_top10'] = get_hot_top10($cat_id);
