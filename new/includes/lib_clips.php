...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, credit_line, last_login, is_validated, people_img, mobile_phone, cardno FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    ...
    $info['people_img']  = $row['people_img'] ;
    $info['mobile_phone']  = $row['mobile_phone'];
    $info['cardno']  = $row['cardno'];
    /* 增加是否开启会员邮件验证开关 */
