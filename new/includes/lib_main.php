...
function update_user_info()
{
    ...
    /* 查询会员信息 */
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, u.mobile_phone, '. //donny
...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money, u.pay_points, u.people_img'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
    ...
    $user['people_img']    = $user['people_img'];
 ...
 * @return  void
 */
function show_message_register($content, $back_act, $auto_redirect = true)
{
    assign_template();
    ...
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->display('message_register.dwt');
