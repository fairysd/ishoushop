...
function edit_profile($profile)
{
    ...
    if ($profile['other']['people_img'])
    {
        if($_FILES['people_img']['size'] / 1024 > $upload_size_limit)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['upload_file_limit'], $upload_size_limit));
            return false;
        }
        $profile['other']['people_img'] = upload_file($_FILES['people_img'], 'people_img');

        $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'people_img');
    }
...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer,people_img,cardno ".
    ...
    $info['people_img'] = $infos['people_img'];
    $info['cardno'] = $infos['cardno'];
